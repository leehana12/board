...
public class BoardDAO {
	...
	public List<BoardVO> selectAll() {
		...
		// java.beans.Statement가 아닌 jdbc를 이용한 데이터 베이스와 상호 작용을 위해서는 java.sql.Statement를
		// 사용한다.
	...
	public int update(BoardVO vo) {
		...
		try {
			...
			pstmt.setString(2, vo.getWriter());
			pstmt.setString(3, vo.getContent()); // 두번쨰 물음표에 vo의 작성자를 설정
			System.out.println("content 값 확인" + vo.getContent());
			pstmt.setInt(4, vo.getNum()); // 세번쨰 물음표에 vo의 내용을 설정
			System.out.println("num 값 확인: " + vo.getNum());
